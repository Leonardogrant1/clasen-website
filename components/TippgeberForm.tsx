...
type Props = {
    dict: Dictionary["baumanagement"]["form"];
};
...
export default function TippgeberForm({ dict }: Props) {
    const [status, setStatus] = useState<'idle' | 'submitting' | 'success' | 'error'>('idle');
    const [type, setType] = useState<string>("bauauftrag");
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (res.ok) {
                trackEvent('tippgeber_form_succeeded', {}, { metaEventName: 'Lead' });
                if (email) posthog.identify(posthog.get_distinct_id(), { email });
                setStatus('success');
            } else {
                trackEvent('tippgeber_form_failed', { status: res.status }, { skipMeta: true });
                setStatus('error');
            }
        } catch {
            trackEvent('tippgeber_form_failed', { status: 'network_error' }, { skipMeta: true });
            setStatus('error');
        }
    }
    ...
    return (
        <form ref={formRef} onSubmit={handleSubmit} className="flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2">
                <label className={labelClass}>{dict.typeLabel} <span className="text-accent">*</span></label>
                <TypeDropdown types={dict.types} value={type} onChange={setType} />
            </div>

            <div className="flex flex-col gap-2">
                <label className={labelClass}>{dict.messageLabel} <span className="text-accent">*</span></label>
                <textarea name="message" rows={4} required placeholder={dict.messagePlaceholder}
